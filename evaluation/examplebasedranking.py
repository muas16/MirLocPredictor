import numpy as np
from evaluation.functions import rankingMatrix, relevantIndexes, irrelevantIndexes
def oneError(y_test, probabilities):
    ...
    oneerror = float(oneerror)/float(len(y_test))
...
def coverage(y_test, probabilities):
    ...
    coverage = float(coverage)/float(len(y_test))
    coverage -= 1.0
