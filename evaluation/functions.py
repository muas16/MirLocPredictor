...
def multilabelConfussionMatrix(y_test, predictions):
    ...
    """
    TP = np.zeros(len(y_test[1]))
    FP = np.zeros(len(y_test[1]))
    TN = np.zeros(len(y_test[1]))
    FN = np.zeros(len(y_test[1]))
...
def rankingMatrix(probabilities):
    ...
    """
    ranking = np.zeros(shape=[len(probabilities), len(probabilities[1])])
